...
p
# render docx
# ggsave('output_data/img/rgdp_yoy_p.png', plot = rgdp_yoy_p) # ,width = 6.69, height = 8.94,
...
df <- read_excel("raw_data/Real GDP Table - FOMC_Dotplot_2017Q4.xlsx",sheet  = "gdp")
...
# ggsave('output_data/img/rgdp_cont_p.png', width = 10, height = 9, dpi = 100) #, width = 16, height = 9, dpi = 100)
...
pic_list$ism_p <- p
p
#  ggsave('output_data/img/ism_p.png', width = 16, height = 9) #, width = 16, height = 9, dpi =
...
# Monetary_Symbols_Names
# select interested variables
select_symbols <- c( "Employment Cost Index: Total compensation: All Civilian"  ,
                     "Average Hourly Earnings of All Employees: Total Private" ) #

select_symbols_names <-  c("Average Hourly Earnings", "Employment Cost Index")


selelct_date <- "2010-01-01"
...
p
# ggsave('output_data/img/laborcost_p.png', width = 16, height = 9) #, width = 16, height = 9, dpi =
...
pic_list$cpi_p <- p
p
# ggsave('output_data/img/cpi_p.png', width = 16, height = 9) #, width = 16, height = 9, dpi =
...
The Federal Open Market Committee (FOMC) raised the target range for the fed funds rate by 0.25% on March 21th, to 1.50% - 1.75%. Since the rate hike was already expected, market participants focused more on the Fed?€™s communication on its future hiking cycle and the ?€œdot plot?€?, which it uses to signal its outlook for future target interest rates.
...
filter( date %in% c( "Mar-2018","Jun-2018") ) %>%
...
df %>% 
  ...
  scale_y_percent( limits=c(0.01 ,0.035), breaks = seq(1,3.5, by =.5)/100) +
...
my_end_date <-  "2018-06-30" %>% as.Date()# tail(rates_data$dates,1)
# three month ago
...
text_df <- text_df %>% mutate(  vjust = if_else( dates == "2017-06-30", +4, -2.2), 
                                hjust = .6 )
...
p <-  ggplot()+
  ...
  labs(x="",y="Breakeven Rate (%)",title="US Inflation Market Expectation",
       subtitle = "5-Year, 5-Year Forward Inflation Expectation Rate" , caption="Source: Federal Reserve")+
  theme_Publication() +
...
rollcorr %>% 
  timetk::tk_tbl(preserve_index = TRUE, rename_index = "dates") %>% 
  ggplot() +
  geom_line( aes( dates, value))
...
my_title <- str_glue("Correlation between {my_benchmark_name[2]} and {my_benchmark_name[2]} Since: {min(df$dates,na.rm=TRUE)}" )
my_caption <- str_glue("Solid line is sector performance, dotted line is {my_benchmark_name} performance\nRed (blue) indicates the cumulative sector performance is higher (lower) than the benchmark")
my_subtitle <- ""
