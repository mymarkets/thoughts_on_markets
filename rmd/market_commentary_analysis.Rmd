...
df <- econ_df %>%
  filter( variables %in% select_symbols_names , dates >= my_start_date ) %>%
...
p <-
  ...
  geom_col(data = df, aes(x=dates, y= values ,fill= values ,color= values ) , 
           alpha= 2/3 ) +
  annotate(geom = "text",
           x = ( text_df$dates + 10) , y = text_df$values ,
           label = scales::percent( text_df$values ) ,
           fontface = 'bold', color = "black" ,  vjust = -3.5, size = 8  ) +
  ...
  labs(x="",y="",
       title="Quarter-to-Quarter Growth in US Real GDP",
...
df <- read_excel("raw_data/Real GDP Table - 2018-Q1.xlsx",sheet  = "gdp")
...
```{r Real GDP Data Components 2, eval=FALSE, cache=TRUE, warning=FALSE,echo=FALSE}
...
# select interested variables
# select_symbols_names <- Real_GDP_Symbols_Names[!Real_GDP_Symbols_Names %in% "Annual Real GDP Growth" ]

select_symbols <- econ_tbl %>%
  filter(category %in% "GDP" ,
         !variables %in% "Annual Real GDP Growth") %>%
  pull(symbol)

###### real GDP data process ######

gdp_data_plot <- econ_df %>%
  ...
  mutate(y_min = pmin(values,0),y_max = pmax(values,0),
         # for plot position purpose
         myjust = ifelse(values < 0 ,1,0)) %>%
  ungroup() # ungroup


gdp_data_plot$id  <- gdp_data_plot %>%
  # convert variables to factor
  mutate(variables = factor(variables,levels = rev(unique(variables )))) %>%
  # process data for geom_rect  for x and y max & min
  group_by(variables) %>%
...
select_date <-  '2017-01-01'

###### Contributions to Percent Change in Real Gross Domestic Product @rgdp ######

# View(gdp_data_plot)
# review last two quarters' changes per component
df <- gdp_data_plot %>% filter(dates >= select_date  )
...
```{r Real GDP Comp timeseries, eval= FALSE, fig.fullwidth = TRUE, fig.height= 12, fig.width= 16,cache=TRUE, warning=FALSE,echo=FALSE}

#### GDP contributions time series @rgdp ####
...
select_date <- '2006-01-01'

df <- gdp_data_plot %>%
  filter(dates >= select_date ) %>%
  mutate(values = values/100)

select_colors_scale <- c(
  nord_palettes$aurora ,
  nord_palettes$frost,
  nord_palettes$moose_pond
...
p <-
  ggplot( data = df )+
  facet_wrap( ~ variables, scales="free_y" ,labeller = label_wrap_gen() ) +
  ...
  geom_rect(data=filter(recession_df,start>=select_date),
            aes(xmin= start, xmax=end, ymin=-Inf, ymax=+Inf), fill="gray", alpha=0.35) +
  geom_line(aes(x=dates,y=values,color=variables),size=1.1) +
  ...
  labs(x="",y="",title="Contributions to Percent Change in Real Gross Domestic Product",
       subtitle="Seasonally adjusted at annual rates",
       caption="Source: U.S. Bureau of Economic Analysis, Table 1.1.2, accessed 8/26/2016.\nShaded area NBER Recession dates") +
  # theme_ipsum_tw() +
  #theme_minimal()+
  theme_Publication()+
...
pic_list$rgdp_cont_time_p <- p

# ggsave('output_data/img/rgdp_cont_time_p', width = 10, height = 9) #, width = 16, height = 9, dpi = 100)
...
cbs_df_sub <- cbs_df_sub %>%
  filter( date >= select_date, date <= end_date) %>%
...
p <-  ggplot()+
  geom_line(data = df , aes(x=dates,y=values, color= variables) , alpha=1,size=1.0) +
...
# select interested variables
select_symbols_names <- c("ISM Manufacturing Index","IP YOY Index") # Conference Board Consumer Confidence SA 1985=100)
select_names <- c("ISM", "Industrial Production")

select_date <- "2010-01-01"
end_date <- "2017-12-31"

p1_title <- "ISM Manufacturing Index"
p2_title <- "Industrial Production"

p_title <- "Economic hope on the rise"
p_subtitle <- "Sentiment and real data continued to go up in fourth quarter of 2016"
p_caption_1 <- "Source: ISM Manufacturing Index is provided by Institute For Supply Management,"
# p_caption_2 <- expression(paste("D.Treisman, ", italic("American Economic Review")," (2016)."))
p_caption_2 <- "Industrial Production provided by Federal Reserve Board."

# date mismatch : ISM, beginning; consumer confidence, end of month
df_1 <-  econ_df %>%
  filter(variables %in% select_symbols_names[1]) %>%
  mutate(year = as.factor(year(dates)), month =
           as.factor(month(dates))) %>%  # add year variable
  group_by(year , month) %>%
  # compute year over year growth rate
  arrange(dates) %>%
  mutate(yoy = (values-lag(values,1))/lag(values,1)) %>%
  filter(dates>= select_date, dates <= end_date  ) %>%
  mutate(pos = yoy) %>%
  ungroup()
...
df_2 <-  bl_all_df %>%
  ...
  filter( variables %in% select_symbols_names[2]) %>%
...
if(!all(df_1$dates %in% df_2$dates)) stop("not valid")

#*****************************************************************
# subset event dates
#******************************************************************

temp_evn_dates <-   c("2008-11-25",
                      "2010-11-02",
                      "2012-09-13" ,
                      "2014-08-29",
                      "2016-02-11" ,
...
temp_evn_df <- data.frame(date =as.Date(temp_evn_dates), event= temp_evn_labels)

# match cbs dates
temp_evn_df_sub <- match_cbs_df(temp_evn_df,df_1)

begin_date <- select_date
end_date <-  tail(temp_evn_df,1)$date

#*****************************************************************
# Graph 1
#******************************************************************
# http://drawar.github.io/posts/dual-y-axis-ggplot2/
# http://stackoverflow.com/questions/37347115/reproduce-a-the-economist-chart-with-dual-axis/37369764



df_1 %>%
  ggplot(aes(x=dates,y=values,fill= variables,color= variables))+
  geom_line(alpha=1,size=1.0,color = select_colors_scale[1])+
  geom_vline(  xintercept = as.numeric(filter(temp_evn_df_sub ,
                                              date >= select_date, date <= end_date)$dates),
               size = 1, color='grey',linetype="dashed")  +
  geom_text_repel(data = filter(temp_evn_df_sub ,date >= select_date, date <= end_date)
                  ,aes(x=  as.Date(dates),y= pos,  label=event),color="#1C444D", fontface = 'bold',family="Arial") +
  # scale_x_continuous("", breaks = c(1996, seq(2000, 2015, 5))) +
  scale_y_continuous("", breaks= c(47,seq(45, 60,2)), expand = c(0, 0)) +
  # ggtitle("ISM Manufacturing Index\n") +
  labs(x="", y= "") +
  ...
  theme(legend.position = "none",
        ...
        axis.text.y = element_text(colour=select_colors_scale[1], size=14),
        axis.text.x = element_text(size = 14),
        axis.ticks = element_line(colour = 'gray50'),
        axis.ticks.length = unit(.25, "cm"),
        axis.ticks.x = element_line(colour = "black"),
        axis.ticks.y = element_line(colour = select_colors_scale[1]),
        #axis.ticks.y = element_blank(),
        axis.line.x =  element_line(colour = "black"),
        plot.title = element_text(hjust = 0.85, vjust=2.12,
                                  colour = select_colors_scale[1], size = 14,
                                  family =  "Arial"),
        panel.border = element_blank(),
        plot.margin = unit(c(50, 10, 40, 10), "pt") )-> p1
...
df_2 %>%
  ggplot(aes(x=dates,y=yoy,fill= variables,color= variables))+
  geom_line(alpha=1,size=1.0,color = select_colors_scale[2]) +
  scale_y_continuous(label=scales::percent)+
  # scale_y_continuous("", breaks= c(103,seq(103, 107,1)), expand = c(0, 0))+
  labs(x=NULL, y= NULL)+
  # theme_ipsum(grid=FALSE,axis = TRUE) -> p2
  theme(
    panel.background = element_blank(),
    panel.grid.minor = element_blank(),
    panel.grid.major = element_blank(),
    text = element_text(family="Arial"),
    axis.text.y = element_text(colour=select_colors_scale[2], size=14),
    axis.text.x = element_text(size=14),
    axis.ticks.y = element_line(colour = select_colors_scale[2]),
    axis.ticks.length = unit(.2, "cm")) -> p2

# p2



my_2yx_plot(p1,p2,p1_title,p2_title,p_title,p_subtitle,
...
scale_color_manual(values = select_colors_scale,labels = rev(select_symbols_names))+
scale_y_continuous(label=scales::percent)+
...
p <- ggplot()+
geom_line(data = df, aes(x=dates,y= values,color= symbol) , alpha=1,size= 1.1 ) +
...
scale_x_date(expand = c(0,0), limits = my_xlim, date_breaks = "1 year" , date_labels = "%Y" ) +
...
temp_key_tbl <- tibble(
symbol =  c("CPIAUCSL" ,"CPILFESL" ,"PCEPILFE") ,
select_names =  factor(c("Headline CPI","Core CPI", "Core PCE"), levels = c("Headline CPI","Core CPI", "Core PCE"))
)
...
filter(dates >= "2001-01-01" , dates <= "2018-03-31") %>%
...
text_df <- filter(df , dates == last(dates) ) %>% 
bind_rows( filter(df ,symbol == "PCEPILFE") %>% filter( dates == last(dates) ) ) %>% 
mutate( values = ann_g)
# View(text_df)

# https://www.ft.com/content/7d97f1b2-e492-11e6-8405-9e5580d6e5fb
p <- df %>%
ggplot(aes(x=dates,y=ann_g,color= select_names ))+geom_line(alpha=1,size=1.1)+
...
scale_x_continuous(breaks=seq(1,4,1),labels=c("2018","2019",
"2020","Longer Run"))+
scale_color_manual(values = select_colors_scale )  + 
...
p <- df %>% 
filter(  date == "Mar-2018", variables <= 2019 , variables >= 2017) %>% 
ggplot() +
# scale_x_discrete(labels=c("2018","2019")) +
geom_line( aes(x = variables ,y = values , color =  group, group =  group),
alpha=1,size=1.1  ) +
scale_color_manual(values = select_colors_scale ) + #labels = select_symbols_names  
guides(colour = guide_legend(reverse=T)) + #  c("#fa9fb5","#2b8cbe")
scale_y_continuous(limits=c(1 ,3.5), breaks = seq(1,3.5, by =.5)) +
...
geom_vline(  xintercept = as.numeric(filter(cbs_df_sub ,date >= select_date, date <= end_date)$dates),
size = 1, color='grey',linetype="dashed")  +
geom_text_repel(data = filter(cbs_df_sub ,date >= select_date, date <= end_date)[1,] ,aes(x=  as.Date(dates),
y= 105, label=event),
size = 5,
family = 'Arial',
fontface = 'bold', color ="#5E81AC",
...
my_end_date <-  "2018-03-31" %>% as.Date()# tail(rates_data$dates,1)
# three month ago
...
key_tbl <- cbind(symbol = Rate_Symbols ,names = Rate_Symbols_Names,
term =Term_Names , maturity = Maturity  ) %>% tbl_df()
...
df <- df %>%
filter(dates >= my_start_date & dates <= my_end_date) %>%
arrange(dates) %>%
mutate( pos = values)

text_df <- df %>% filter(term %in% "3 Year") %>% distinct(.keep_all = TRUE)

text_df <- text_df %>% mutate( dates = lubridate::ceiling_date(dates, "month") - days(1))
text_df <- text_df %>% mutate(  vjust = if_else( dates == "2017-06-30", +4.6, -2.1), 
hjust = .6 )
...
scale_y_percent()+
coord_cartesian(xlim = c(0., 30),ylim = c(0, .032)) +
annotate("text",  x = text_df$maturity , y = text_df$cmt ,
label = text_df$dates, color =select_colors_scale ,
# size = 7,
fontface = "bold", cex = 4.5,  hjust = text_df$hjust, vjust = text_df$vjust) +
scale_color_manual(values = select_colors_scale) +
labs(x="Time To Maturity",y="Yield to Maturity (%)",
subtitle = "Treasury Yield Curve Quarter Over Quarter Comparison For The First Quarter 2018",
title="US Treasury Yield Curve",
caption="Source: Daily Treasury Yield Curve Rates, US Department of the Treasury",
color = "Yield Date") +
theme_Publication() + theme(axis.text  = element_text(size=12))+
theme(legend.position = "none") +
theme(
panel.grid.major = element_line(colour = "grey92"),
panel.grid.minor = element_line(colour = "grey92",
...
ggsave('output_data/img/yld_curv_p.png') #, width
...
select_symbols_names <- c(
 "10-Year Breakeven Inflation Rate") #  '5/5 Year Forward Inflation Expectation'
...
scale_y_continuous(label=scales::percent)+
labs(x="",y="Breakeven Rate (%)",title="US Inflation Market Expectation",
subtitle = "10-Year Breakeven Inflation Rate"  , caption="Source: Federal Reserve")+ # "5-Year, 5-Year Forward Inflation Expectation Rate"
theme_Publication() +
...
pic_list$tytb_p <- p
p
ggsave('output_data/img/tytb_p.png') #, width = 16, height = 9, dpi =
...
my_symbols <-  c(  'USGG2YR Index',    'USGG10YR Index')
my_symbols_names <- c("2-Year CMT"  ,"10-Year CMT"  ) 
...
df <- bloomberg_df %>% 
  ...
  left_join(
    tibble( my_symbols_names ,my_symbols ) , 
    by  = c("symbol" = "my_symbols")
  ) %>%
  distinct(my_symbols_names, dates, .keep_all =  TRUE) %>% 
  filter( between( dates ,my_start_date, my_end_date), !is.na(values)  ) %>%
  select(dates,values,my_symbols_names) %>%
  spread(my_symbols_names  , values) %>%
...
my_symbols <-  c( 'SPX Index' ,  'LT09TRUU Index')
my_symbols_names <- c("S&P 500 Index" , "Bloomberg Barclays U.S. Treasury: 7-10")
...
df <- bloomberg_df %>% 
  filter(  symbol %in% my_symbols   ) %>% 
  distinct(symbol, dates, .keep_all =  TRUE) %>% 
  group_by(symbol) %>%
  tq_transmute(select = values ,  mutate_fun = periodReturn, period = "daily") %>% 
  rename_(.dots=setNames(names(.), c('symbol','dates','values'))) %>% 
  ungroup() %>% 
  #  Key table to join human readable names to data frame
  left_join(
...
# https://stackoverflow.com/questions/48693743/multiple-rolling-correlation
...
df <- rolling_correlation(df, c("SPX Index"), c("LT09TRUU Index"),21)


df <-  df %>% select( dates, 4) %>% set_names(c("dates", "values")) 
...
df <-  df  %>%  filter( between(dates, my_start_date , my_end_date) ) # my_start_date

#### Prepare titles ################################################################


my_title <- str_glue("Correlation between {my_symbols_names[2]} and {my_symbols_names[2]} Since: {min(df$dates,na.rm=TRUE)}" )
my_caption <- str_glue("Solid line is sector performance, dotted line is {my_symbols_names[1]} performance\nRed (blue) indicates the cumulative sector performance is higher (lower) than the benchmark")
my_subtitle <- ""
...
corr_p  <- ggplot() +
  geom_col(data = df  ,
           aes(x=dates, y= values ,fill= values ,color= values ),alpha= 2/3 ) +
  annotate(geom = "text",
           x = text_df$dates , y = text_df$values ,
           label = scales::percent( text_df$values ) ,
           fontface = 'bold', color = "black" ,  vjust = -.5, size = 10) +
# scale_y_percent(limits= my_ylim ) +  #use scale_y_percent function from hrbrtheme
  scale_x_date(limits = my_xlim,date_breaks = "1 year" , date_labels = "%Y" ) +
  labs(x="",y="",
       title= my_title ,
